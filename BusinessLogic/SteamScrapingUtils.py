...
class SteamScraper(BaseScraper):
    ...
    def get_steam_id_to_user_dict(self, users):
        steam_id_to_user=dict()
        for user in users:
            steam_id_to_user[user.steam_id] = user.user_name
        return steam_id_to_user

    def verify_after_n_giveaways(self, steam_after_n_thread_link, giveaways, group_users, max_pages=0):
        after_n_giveaways = dict()
        steam_id_to_user = self.get_steam_id_to_user_dict(group_users.values())
        page_index = 1
        while page_index < max_pages or max_pages == 0:
            page_content = self._get_page_content(steam_after_n_thread_link + steam_search_query + str(page_index))
            ...
            gifter_elemens = self._get_items_by_xpath(html_content, u'.//div[@class="commentthread_comment responsive_body_text  "]')
            for gifter_elem in gifter_elemens:
                steam_user = self._get_item_by_xpath(gifter_elem, u'.//a[@class="hoverunderline commentthread_author_link "]/@href')
                if not steam_user:
                ...
                if steam_profile_link in steam_user:
                    ...
                    page_content = self._get_page_content(steamfinder_lookup_link + steam_user.split(steam_user_id_link)[1])
                ...
                if steam_id in steam_id_to_user:
                    ...
                    giveaway_links = self._get_items_by_xpath(gifter_elem,u'.//div[@class="commentthread_comment_text"]/a/@href')
    ...
    def get_user_steam_id(self, user):
        html_content = html.fromstring(self._get_page_content(SteamGiftsScrapingUtils.get_user_link(user)))
        steam_user = self._get_item_by_xpath(html_content, u'.//div[@class="sidebar__shortcut-inner-wrap"]/a/@href')
