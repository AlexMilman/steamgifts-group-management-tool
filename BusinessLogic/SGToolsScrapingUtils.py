...
class SGToolsScraper(BaseScraper):
    ...
    def check_level(self, user, level):
        user_html_content = html.fromstring(self._get_page_content(SteamGiftsScrapingUtils.get_user_link(user)))
        user_level_item = self._get_item_by_xpath(user_html_content, u'.//div[@class="featured__table__row__right"]/span/@data-ui-tooltip')
    ...
    def check_real_cv_value(self, user):
        sent_html_content = html.fromstring(self._get_page_content(sgtools_check_sent_link + user))
        sent_value = self._get_item_by_xpath(sent_html_content, u'.//div[@class="total"]/h1/text()').replace('$', '')
        won_html_content = html.fromstring(self._get_page_content(sgtools_check_won_link + user))
        won_value = self._get_item_by_xpath(won_html_content, u'.//div[@class="total"]/h1/text()').replace('$', '')
        return StringUtils.normalize_float(won_value) > StringUtils.normalize_float(sent_value)


    def check_multiple_wins(self, user):
        return self._get_page_content(sgtools_check_multiple_wins_link + user).find(' has no multiple wins for the same game!') == -1


    def check_nonactivated(self, user):
        nonactivated_page_content = self._get_page_content(sgtools_check_nonactivated_link + user)
